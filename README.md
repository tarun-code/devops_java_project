...
```bash
curl -O https://s3.ap-south-1.amazonaws.com/amazon-eks/1.27.1/2023-04-19/bin/linux/amd64/kubectl
chmod +x ./kubectl 
mv kubectl /bin
```
...
```bash
eksctl create cluster --name Web-Project-cluster \
--region ap-south-1 \
--node-type t2.small
```
...
```bash
aws eks update-kubeconfig --region ap-south-1 --name Web-Project-cluster
kubectl get nodes
```
